<template>
	<view>
			...
			<view class="moments__post" v-for="(post,index) in articleList" :key="index" :id="'post-'+index">
				...
				<view class="post_right">
					...
					<view class="thumbnails" v-if="post.fileList.length>0">
						<view v-if="post.fileList.length === 1" :style="{width:`${post.fileList[0].width}upx`, height:`${post.fileList[0].height}upx`}"
						 class="my-gallery">
							<image class="gallery_img" lazy-load mode="aspectFill" @load="imageLoad($event,post.fileList[0])" :style="{width:`${post.fileList[0].width}upx`, height:`${post.fileList[0].height}upx`}"
							 :src="post.fileList[0].fileUrl" :data-src="post.fileList[0].fileUrl" @tap="previewImage(post.fileList,0)">
							</image>
						</view>
						<template v-else>
							<view class="thumbnail" v-for="(image, index_images) in post.fileList" :key="index_images">
								<image class="gallery_img" lazy-load mode="aspectFill" :src="image.fileUrl" :data-src="image.fileUrl" @tap="previewImage(post.fileList,index_images)">
								</image>
							</view>
						</template>
					...
					<view class="post-footer">
						...
						<view class="footer_content" v-for="(comment,comment_index) in post.commentList" :key="comment_index" @tap="reply(post.id,comment)">
							<text v-if="!comment.commentParentClanManName" class="comment-nickname">{{comment.clanManName}}
		...
		<cl-popup :visible.sync="visible" direction="bottom">
			...
			<cl-input style="width: 100%;" maxlength="100" v-model="currComment.commentContent" placeholder="请输入(100字以内)" type="textarea">
			</cl-input>
			<view class="mb30">
			</view>
